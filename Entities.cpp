...
void snakeCell(COORD c, SDL_bool head, int bodyColor)
{
	int color = head ? CR_SNAKE_HEAD : bodyColor;
...
{
	snakeCell(s->coords[0], SDL_TRUE, s->bodyColor);
	for (int i = 1; i < s->len; i++) {
		snakeCell(s->coords[i], SDL_FALSE, s->bodyColor);
	}
}
...
void notFruitDraw(const NOTFRUIT * f)
{
	rectDraw(f->loc.x * BOARD_CELL, f->loc.y * BOARD_CELL,
	BOARD_CELL, BOARD_CELL, CR_RED);
}

void notFruitGen(NOTFRUIT * f, const SNAKE * s)
{
	SDL_bool ok = SDL_FALSE;
	int i;
	while (!ok) {
